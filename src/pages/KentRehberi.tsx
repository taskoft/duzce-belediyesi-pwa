...
import { InfrastructureDirectory } from "@/components/kentRehberi/InfrastructureDirectory";
...
const TABS: { id: KentRehberiTab; label: string }[] = [
  ...
  { id: "infrastructure", label: "Kent Altyapısı" },
];

const INDICATOR_TRANSFORMS: Record<KentRehberiTab, string> = {
  corporate: "translate-x-0",
  tourism: "translate-x-[calc(100%+2.667px)]",
  infrastructure: "translate-x-[calc(200%+5.333px)]",
};
      ...
      <main className="absolute inset-0 overflow-y-auto pb-[76px] pt-[56px]">
        <div className="sticky top-0 z-10 bg-background-subtle/90 px-container-margin py-stack-md backdrop-blur-md">
          <div className="relative flex rounded-xl bg-surface-container-low p-1">
            <div
              className={`absolute inset-y-1 left-1 w-[calc(33.333%-2.667px)] rounded-lg bg-surface shadow-sm transition-transform duration-300 ${INDICATOR_TRANSFORMS[activeTab]}`}
            />
            {TABS.map((tab) => (
              <button
                key={tab.id}
